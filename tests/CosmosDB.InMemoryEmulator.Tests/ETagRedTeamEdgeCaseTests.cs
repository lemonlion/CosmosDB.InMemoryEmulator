...
/// </summary>
public class ETagRedTeamEdgeCaseTests
{
    private readonly InMemoryContainer _container = new("etag-redteam", "/partitionKey");
    ...
    [Fact]
    public async Task IfMatchEtag_EmptyString_ShouldReject()
    {
        var create = await _container.CreateItemAsync(
            new TestDocument { Id = "fmt-1", PartitionKey = "pk1", Name = "Original" },
            new PartitionKey("pk1"));

        // Empty string is not a valid ETag and should not match
    ...
    [Fact]
    public async Task IfMatchEtag_WhitespaceOnly_ShouldReject()
    {
        var create = await _container.CreateItemAsync(
    ...
    [Fact]
    public async Task IfMatchEtag_WithoutQuotes_ShouldReject()
    {
        var create = await _container.CreateItemAsync(
            new TestDocument { Id = "fmt-3", PartitionKey = "pk1", Name = "Original" },
            new PartitionKey("pk1"));

        // ETags are generated as quoted hex strings; unquoted version should not match
    ...
    public async Task IfMatchEtag_WildcardStar_OnDeletedItem_ReplaceShouldFail()
    {
        ...
        await _container.DeleteItemAsync<TestDocument>("wild-2", new PartitionKey("pk1"));
    ...
    public async Task IfMatchEtag_WildcardStar_OnDeletedItem_DeleteShouldFail()
    {
        ...
        await _container.DeleteItemAsync<TestDocument>("wild-3", new PartitionKey("pk1"));
    ...
    [Fact]
    public async Task IfMatchEtag_WildcardStar_UpsertNonExistentItem_ShouldReturnNotFound()
    {
        // Upsert with IfMatchEtag on non-existent item: Cosmos returns NotFound
        var act = () => _container.UpsertItemAsync(
    ...
    public async Task DeleteThenReplace_WithOldETag_ShouldFail()
    {
        ...
        await _container.DeleteItemAsync<TestDocument>("del-1", new PartitionKey("pk1"));
    ...
    [Fact]
    public async Task DeleteThenUpsert_WithOldETag_ShouldFail()
    {
        var create = await _container.CreateItemAsync(
        ...
        await _container.DeleteItemAsync<TestDocument>("del-2", new PartitionKey("pk1"));

        // Upsert with IfMatchEtag on deleted item should be NotFound
        var act = () => _container.UpsertItemAsync(
    ...
    public async Task DeleteThenRecreate_OldETag_ShouldNotMatchNewItem()
    {
        ...
        await _container.DeleteItemAsync<TestDocument>("del-3", new PartitionKey("pk1"));

        // Recreate same item
        var recreate = await _container.CreateItemAsync(
    ...
    {
        var createA = await _container.CreateItemAsync(
            new TestDocument { Id = "xpk-2", PartitionKey = "pkA", Name = "Item A" },
            new PartitionKey("pkA"));

        var createB = await _container.CreateItemAsync(
        ...
        var replaceB = await _container.ReplaceItemAsync(
    ...
    public async Task Patch_ETagChangesAfterPatch()
    {
        ...
        var patchResult = await _container.PatchItemAsync<TestDocument>(
    ...
    [Fact]
    public async Task StreamUpsert_OnNonExistentItem_WithETag_ShouldReturnNotFound()
    {
        using var stream = new MemoryStream(Encoding.UTF8.GetBytes(
    ...
    public async Task StreamReplace_OnDeletedItem_WithOldETag_ShouldReturnNotFound()
    {
        ...
        await _container.DeleteItemAsync<TestDocument>("stream-5", new PartitionKey("pk1"));
    ...
    [Fact]
    public async Task StreamUpsert_Wildcard_OnNonExistentItem_ShouldReturnNotFound()
    {
        // With IfMatchEtag set (even wildcard), Cosmos requires item to exist for Upsert
        using var stream = new MemoryStream(Encoding.UTF8.GetBytes(
    ...
    public async Task ReadAfterReplace_ShouldReturnNewETag()
    {
        ...
        var read = await _container.ReadItemAsync<TestDocument>("conc-3", new PartitionKey("pk1"));
    ...
    public async Task IfNoneMatch_WithStaleETag_Read_ShouldSucceed()
    {
        ...
        var read = await _container.ReadItemAsync<TestDocument>(
    ...
    [Fact]
    public async Task IfNoneMatch_Wildcard_Patch_OnExistingItem_ShouldReturn412()
    {
        await _container.CreateItemAsync(
    ...
    public async Task Batch_MixedOperations_OneStaleETag_ShouldRollbackAll()
    {
        ...
        var batch = _container.CreateTransactionalBatch(new PartitionKey("bpk"));
        ...
        var read1 = await _container.ReadItemAsync<TestDocument>("batch-1", new PartitionKey("bpk"));
    ...
    public async Task Batch_Delete_WithStaleETag_ShouldRollback()
    {
        ...
        var read = await _container.ReadItemAsync<TestDocument>("batch-3", new PartitionKey("bpk"));
        read.Resource.Name.Should().Be("Updated");
    }
    ...
    public async Task ImportState_RegeneratesETags_OldETagsShouldNotWork()
    {
        ...
        // Export and reimport
        var state = _container.ExportState();
        _container.ImportState(state);
    ...
    public async Task ImportState_NewETagsShouldWork()
    {
        ...
        var read = await _container.ReadItemAsync<TestDocument>("import-2", new PartitionKey("pk1"));
    ...
    public async Task ImportState_EmptyImport_ShouldClearEverything()
    {
        ...
        _container.ImportState("{\"items\":[]}");

        var act = () => _container.ReadItemAsync<TestDocument>("import-3", new PartitionKey("pk1"));
        var ex = await act.Should().ThrowAsync<CosmosException>();
        ex.Which.StatusCode.Should().Be(HttpStatusCode.NotFound);
    }
    ...
    public async Task ETag_ReturnedFromCreate_MatchesSubsequentRead()
    {
        ...
        var read = await _container.ReadItemAsync<TestDocument>("resp-1", new PartitionKey("pk1"));
    ...
    public async Task ETag_ReturnedFromUpsert_MatchesSubsequentRead()
    {
        ...
        var read = await _container.ReadItemAsync<TestDocument>("resp-2", new PartitionKey("pk1"));
    ...
    public async Task ETag_ReturnedFromReplace_MatchesSubsequentRead()
    {
        ...
        var read = await _container.ReadItemAsync<TestDocument>("resp-3", new PartitionKey("pk1"));
    ...
    public async Task ETag_ReturnedFromPatch_MatchesSubsequentRead()
    {
        ...
        var read = await _container.ReadItemAsync<TestDocument>("resp-4", new PartitionKey("pk1"));
    ...
    public async Task ETag_StreamRead_MatchesTypedRead()
    {
        ...
        var typedRead = await _container.ReadItemAsync<TestDocument>("resp-5", new PartitionKey("pk1"));
        var streamRead = await _container.ReadItemStreamAsync("resp-5", new PartitionKey("pk1"));
    ...
    public async Task ETags_AreUniqueAcrossMultipleOperations()
    {
        ...
        for (int i = 0; i < 50; i++)
        {
            var resp = await _container.UpsertItemAsync(
    ...
    public async Task DoubleDelete_WithSameETag_SecondShouldFail()
    {
        ...
        await _container.DeleteItemAsync<TestDocument>(
    ...
    public async Task DoubleReplace_WithFirstETag_SecondShouldFail()
    {
        ...
        var replace1 = await _container.ReplaceItemAsync(
    ...
    [Fact]
    public async Task Upsert_AsCreate_WithIfMatchEtag_ShouldReturnNotFound()
    {
        // Upsert that would create a new item, but IfMatchEtag is set
        // Real Cosmos returns 404 because there's no existing item to match against
        var act = () => _container.UpsertItemAsync(
    ...
    public async Task OptimisticConcurrency_ReadModifyWrite_ShouldWork()
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            var read = await _container.ReadItemAsync<TestDocument>("occ-1", new PartitionKey("pk1"));
            var item = read.Resource;
            item.Value++;
            item.Name = $"v{item.Value}";

            var response = await _container.ReplaceItemAsync(item, "occ-1", new PartitionKey("pk1"),
                new ItemRequestOptions { IfMatchEtag = read.ETag });
            response.StatusCode.Should().Be(HttpStatusCode.OK);
        }

        var final = await _container.ReadItemAsync<TestDocument>("occ-1", new PartitionKey("pk1"));
    ...
    public async Task ETag_InDocument_MatchesResponseHeader()
    {
        ...
        var streamRead = await _container.ReadItemStreamAsync("sysprop-1", new PartitionKey("pk1"));
    ...
    [Fact]
    public async Task Patch_WithBothIfMatchAndIfNoneMatch_MatchingETag_ShouldFail()
    {
        // If both are set to the current ETag:
        // - IfMatch succeeds (etag matches)
        // - IfNoneMatch fails (etag matches => 412 for writes)
        var create = await _container.CreateItemAsync(
        ...
        // IfNoneMatch should cause 412 even though IfMatch passes
        var ex = await act.Should().ThrowAsync<CosmosException>();
        ex.Which.StatusCode.Should().Be(HttpStatusCode.PreconditionFailed);
    }
    ...
    {
        var v1 = await _container.CreateItemAsync(
            new TestDocument { Id = "chain-1", PartitionKey = "pk1", Name = "v1" },
            new PartitionKey("pk1"));

        var v2 = await _container.ReplaceItemAsync(
        ...
        var v3 = await _container.ReplaceItemAsync(
        ...
        var act2 = () => _container.ReplaceItemAsync(
        ...
        // v3 ETag should work
        var v4 = await _container.ReplaceItemAsync(
    ...
    public async Task RestoreToPointInTime_RegeneratesETags_OldETagsShouldNotWork()
    {
        ...
        _container.RestoreToPointInTime(DateTimeOffset.UtcNow);
    ...
    public async Task RestoreToPointInTime_NewETags_ShouldWork()
    {
        ...
        // Read to get new ETag
        var read = await _container.ReadItemAsync<TestDocument>("pitr-2", new PartitionKey("pk1"));
