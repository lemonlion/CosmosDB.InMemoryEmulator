...
public class RealToFeedIteratorStringTests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task ToFeedIterator_WithStringLength_Divergent_ReturnsAllItemsBecauseLengthIgnored()
    {
        // DIVERGENT BEHAVIOUR: Real Cosmos DB correctly evaluates LENGTH() in WHERE clauses
        // generated by LINQ. The in-memory emulator's SQL parser does not support LENGTH()
        // so when queried via raw SQL with LENGTH(), the filter may be effectively ignored.
        // This tests the actual emulator behaviour through a simple equality query instead.
        await SeedAsync(
            new TestDocument { Id = "1", PartitionKey = "pk", Name = "Al" },
            new TestDocument { Id = "2", PartitionKey = "pk", Name = "Alice" },
            new TestDocument { Id = "3", PartitionKey = "pk", Name = "Bob" });
