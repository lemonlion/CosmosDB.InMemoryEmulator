...
// ─── F1. Full-Text Search Functions ─────────────────────────────────────

public class FullTextSearchTests
{
    /// <summary>
    /// Full-text search functions (FULLTEXTCONTAINS, FULLTEXTCONTAINSALL, FULLTEXTCONTAINSANY)
    /// require a specialized full-text index on the container. This is a server-side feature
    /// that involves tokenization, stemming, and stop-word removal — capabilities that are
    /// well beyond the scope of an in-memory emulator.
    /// </summary>
    [Fact(Skip = "Full-text search functions (FULLTEXTCONTAINS, FULLTEXTCONTAINSALL, " +
        "FULLTEXTCONTAINSANY) require a specialized full-text index with tokenizer, stemmer, " +
        "and stop-word configuration. These are server-side capabilities that require NLP " +
        "infrastructure. Implementing even a basic version would need a text analysis pipeline. " +
        "Use CONTAINS() or REGEXMATCH() as workarounds for simple text matching.")]
    public async Task FullTextContains_ShouldSearchWithTextIndex()
    {
        var container = new InMemoryContainer("test-container", "/pk");
        await container.CreateItemAsync(
            JObject.FromObject(new { id = "1", pk = "a", description = "The quick brown fox jumps" }),
            new PartitionKey("a"));
...
// ─── F2. FULLTEXTSCORE + ORDER BY RANK ──────────────────────────────────

public class FullTextScoreTests
{
    /// <summary>
    /// FULLTEXTSCORE computes a BM25 relevance score for full-text search queries and can
    /// only be used with ORDER BY RANK. This requires the same NLP/indexing infrastructure
    /// as the other full-text search functions.
    /// </summary>
    [Fact(Skip = "FULLTEXTSCORE returns a BM25 relevance score and requires full-text indexing " +
        "infrastructure. ORDER BY RANK is a special clause that sorts by computed relevance " +
        "scores. Both require NLP tokenization and term-frequency analysis that is not " +
        "feasible in an in-memory emulator. Use ORDER BY with a computed similarity field " +
        "as a workaround.")]
    public async Task FullTextScore_WithOrderByRank_ShouldSortByRelevance()
    {
        var container = new InMemoryContainer("test-container", "/pk");
        await container.CreateItemAsync(
            ...
            new PartitionKey("a"));
        await container.CreateItemAsync(
            JObject.FromObject(new { id = "2", pk = "a", text = "Cosmos DB is a database service from Azure" }),
            new PartitionKey("a"));
